...
Static website for **"Olhares Capixabas"**, a photography exposition by 6 photographers from Espírito Santo, Brazil, running September 1–30 at the Tribunal de Justiça do Espírito Santo (Vitória), curated by Profa. Kátia Ludolf. Live domain: <https://mostraolharescapixabas.art.br>.
...
**Homepage — `index.md`.** Frontmatter keys: `title`, `period`, `tagline`, `venue`, `address`, `curator`, `url`, `start`. The body is an optional introduction text. The `url` key enables canonical URLs, `og:image`, sitemap and robots.txt — remove it and those disappear, nothing breaks.

**Photo embargo — `start` (ISO date, e.g. `2026-08-01`).** Building before that date keeps the photos out of `dist/` entirely and renders framed placeholder tiles with a "fotografias a partir de…" note instead; building on or after it publishes the real galleries. The site is static, so the switch happens at build time, not in the browser. Remove the key to always publish.

The series photos themselves are not committed in the clear before the opening (public repo): each photographer's series lives in `private/<slug>-publish.patch` (local only, gitignored) and as an encrypted copy `private/<slug>-publish.patch.enc` (committed; AES-256-CBC, PBKDF2, passphrase in `private/patches.key`, local only, mirrored in the `PATCHES_KEY` Actions secret). From opening day on, every deploy run decrypts and `git apply`s the patches before building — the daily cron flips the embargo unattended. After the opening, commit the applied photos in the clear, then delete the `.enc` files and the decrypt step, so the repo is self-sufficient again.
...
Black editorial theme inspired by gallery/print design: near-black background, warm off-white ink, [Fraunces](https://fonts.google.com/specimen/Fraunces) display serif (the site's only external resource, loaded from Google Fonts with a graceful serif fallback), and tiny uppercase letterspaced labels used as "metadata" throughout. Titles and names get an automatic italic accent on the last word ("Olhares *Capixabas*"). Each photographer also gets an accent color (evenly-spaced hues derived from the photographer count in `build.py`) used sparingly — roster number, hover, italic surname, figure captions — and the homepage title's italic word slowly cycles through the whole wheel.

The homepage is a poster: oversized title with the curator credit at its baseline, a Quando/Onde info band (address linked to Google Maps), and a numbered photographer roster with hairline rules. Photographer pages set the statement lede large, then run the gallery in a repeating asymmetric rhythm — full-width, large-left, smaller-right-offset — with numbered captions. Portrait images are capped at 85vh; clicking a photo opens it full-screen in a CSS `:target` lightbox with a bottom thumbnail navigator (click the photo to close — not present in pre-opening builds). The only JavaScript on the site is a ~10-line inline keyboard handler for the lightbox (←/→ navigate with wrap-around, Esc closes); everything else is CSS. Everything collapses to a single column on narrow screens.
